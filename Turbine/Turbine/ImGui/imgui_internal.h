...
static inline float  ImLinearSweep(float current, float camTarget, float speed)    { if (current < camTarget) return ImMin(current + speed, camTarget); if (current > camTarget) return ImMax(current - speed, camTarget); return current; }
